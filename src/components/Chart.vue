	...
	export default {
		...
		methods: {
			...
			drawBars(ctx, coordinateSystem) {
				...
				const max = Math.max.apply(null, this.chartOptions.series);
				...
				for (let i = 0; i < count; i++) {
					const height = coordinateSystem.height * this.chartOptions.series[i] / (stepCount * step);
					ctx.fillStyle = 'rgb(187, 44, 40)';
					ctx.fillRect(coordinateSystem.start.x + i * itemWidth + itemPadding, coordinateSystem.start.y - height,
						itemWidth - itemPadding * 2, height);

					ctx.fillStyle = '#000';
					ctx.fillText(this.chartOptions.xAxis[i], coordinateSystem.start.x + i * itemWidth + itemWidth / 2,
						coordinateSystem.start.y + 20);
				}
